...
namespace muphic.MakeStory
{
	...
	public class BirdButton : Base
	{
		...
		public BirdButton(MakeStoryScreen mss)
		{
			parent = mss;
		}
		...
		{
			base.Click (p);
			if(parent.ButtonsMode == muphic.MakeStory.ButtonsMode.Sparrow)
			{
			...
			else
			{
				if(parent.isClear)
					parent.cb.Reset();
				parent.ButtonsMode = muphic.MakeStory.ButtonsMode.Sparrow;
				this.State = 1;
				parent.man.State = 0;
				parent.lady.State = 0;
				parent.girl.State = 0;
