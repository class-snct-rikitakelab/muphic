...
{
	/// <summary>
	/// ver1.0.0 ChangeScroll追加 ScrollBarを部品として追加
	/// ver1.1.0 DragBegin,Drag追加
	/// </summary>
...
//			}else
			if(parent.ones.NowClick != muphic.One.OneButtonsClickMode.None)		//右のボタン群で何かを選択していれば以下を実行する
			{
				if(parent.ones.NowClick == muphic.One.OneButtonsClickMode.Cancel)//キャンセルボタンがクリックされていたら
				{
				...
				{
					bool b = Animals.Insert(place.X, place.Y, parent.ones.NowClick.ToString());//挿入処理を実行
		...
		{
			base.DragEnd (begin, p);		//Draw時と同じ処理(但し、Replace時の音は鳴らす)
