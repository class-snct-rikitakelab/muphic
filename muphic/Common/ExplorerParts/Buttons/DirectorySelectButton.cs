...
{
	/// <summary>
	/// エクスプローラ上のディレクトリを選択するボタン。
	/// </summary>
	public class DirectorySelectButton : Button
	{
		/// <summary>
		/// 親にあたるディレクトリ選択領域。
		/// </summary>
		...
		/// <summary>
		/// エクスプローラ上のディレクトリ選択ボタンの新しいインスタンスを初期化する。
		/// </summary>
		/// <param name="parent">親にあたるディレクトリ選択領域。</param>
		/// <param name="num">選択領域上で上から何番目のボタンかを示す整数。</param>
		public DirectorySelectButton(DirectorySelectArea parent, int num)
		{
			this.Parent = parent;
			this.Num = num;

			this.Location = Settings.GetDirecyorySelectButtonLocation(num);
